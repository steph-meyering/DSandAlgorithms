...
def color_graph(graph, colors):
    ...
    # get set of n + 1 colors
    # each node:
    # get colors of neighbors
    # assign color unused color to node
    #

    for node in graph:
        c = set(colors[:len(node.neighbors)+1])
        for neighbor in node.neighbors:
            if neighbor == node:
                raise InputError("Node can't point to itself")
            if neighbor.color:
                c.remove(neighbor.color)
        node.color = c.pop()
